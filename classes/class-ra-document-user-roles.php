...
class RA_Document_User_Roles {
	...
	*/
	function __construct() {
		if ( class_exists( 'RA_Document_Post_Type' ) ) {
			...
			$this->post_type_name = $ra_document_library->post_type_name;
		}
	...
	*/
	function admin_page() {
		...
		<form method="POST">
			<table class="widefat">
				<thead>
				<td><?php _e( 'ID', 'document-repository' ); ?></td>
				<td><?php _e( 'Description', 'document-repository' ); ?></td>
				<td><?php _e( 'Actions', 'document-repository' ); ?></td>
				</thead>
				<tbody><?php
				if ( ! empty( $roles ) ) {
			...
			<p><?php _e( 'Add Document Role', 'document-repository' ); ?>&nbsp;<input type='text' name='new_role'
																					  value=''/></p>
	...
	function add_admin_page() {
		add_submenu_page( 'edit.php?post_type=' . $this->post_type_name, __( 'Document Roles', 'document-repository' ), __( 'Document Roles', 'document-repository' ), 'manage_options', 'ra_doc_roles_admin', array(
			&$this,
			'admin_page'
		) );
	}
	...
	*/
	function personal_options( $user ) {
		...
		?>
		<tr>
		<th scope="row"><?php _e( 'Document Role', 'document-repository' ); ?></th>
